...
function Card(item: BlogsPageDataType) {
  return (
    <>
      <div className='border border-primary-500 rounded-3xl p-4 group hover:bg-primary-500 transition-all duration-500 cursor-pointer'>
        ...
        <div className='mt-5 flex items-start justify-start gap-5 flex-col'>
          ...
          <p className='font-ppMori text-[12px]'>{item.summary}</p>
          <p className='font-ppMori text-[10px]'>{item?.date} | {item?.read_time}</p>
        </div>
      </div>
    </>
  )
